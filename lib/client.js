...
export class RealtimeClient extends RealtimeEventHandler {
  ...
  constructor({ url, apiKey, dangerouslyAllowAPIKeyInBrowser, debug } = {}) {
    ...
    this.defaultSessionConfig = {
      ...
      instructions: '',
      audio: {
        input: { format: 'pcm16', turn_detection: null},
        output: { format: 'pcm16', voice: 'verse' },
      },
      tools: [],
      tool_choice: 'auto',
  ...
  getTurnDetectionType() {
    return this.sessionConfig.audio.turn_detection?.type || null;
  }
  ...
  updateSession({
    ...
    output_modalities !== void 0 && (this.sessionConfig.output_modalities = output_modalities);
    instructions !== void 0 && (this.sessionConfig.instructions = instructions);
    if (audio !== void 0) {
      if (!this.sessionConfig.audio) {
        this.sessionConfig.audio = {};
      }
      if (audio.input !== void 0) {
        this.sessionConfig.audio.input = { ...this.sessionConfig.audio.input, ...audio.input };
      }
      if (audio.output !== void 0) {
        this.sessionConfig.audio.output = { ...this.sessionConfig.audio.output, ...audio.output };
      }
    }
    tools !== void 0 && (this.sessionConfig.tools = tools);
    tool_choice !== void 0 && (this.sessionConfig.tool_choice = tool_choice);
    max_output_tokens !== void 0 &&
      (this.sessionConfig.max_output_tokens = max_output_tokens);
  ...
  cancelResponse(id, sampleCount = 0) {
    if (!id) {
      ...
      this.realtime.send('response.cancel');
      const audioIndex = item.content.findIndex((c) => c.type === 'output_audio');
      if (audioIndex === -1) {
