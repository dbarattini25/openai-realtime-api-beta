...
export class RealtimeConversation {
  ...
  EventProcessors = {
    'conversation.item.added': (event) => {
      const { item } = event;
      console.log('[DEBUG] conversation.item.added - Item ID:', item.id, 'Role:', item.role, 'Type:', item.type);
      console.log('[DEBUG] Item has content:', !!item.content, 'Content length:', item.content?.length);
      // deep copy values
      const newItem = JSON.parse(JSON.stringify(item));
      if (!this.itemLookup[newItem.id]) {
        console.log('[DEBUG] Adding new item to lookup:', newItem.id);
        this.itemLookup[newItem.id] = newItem;
        this.items.push(newItem);
      } else {
        console.log('[DEBUG] Item already exists in lookup:', newItem.id);
      }
      console.log('[DEBUG] Items in lookup after add:', Object.keys(this.itemLookup));
      // Ensure content array exists for GA API
      if (!newItem.content) {
        newItem.content = [];
      }
      newItem.formatted = {};
    ...
    'conversation.item.done': (event) => {
      ...
      if (!existingItem) {
        throw new Error(`conversation.item.done: Item "${item.id}" not found`);
      }
      // Update the item with final status and any final content
      existingItem.status = item.status;
      // Update content if it has changed (e.g., final transcript, final audio)
      if (item.content) {
        existingItem.content = item.content;
        // Update formatted text for any new text content
        existingItem.formatted.text = '';
        const textContent = item.content.filter((c) =>
          ['text', 'input_text', 'output_text'].includes(c.type),
        );
        for (const content of textContent) {
          if (content.text) {
            existingItem.formatted.text += content.text;
          }
        }
      }
    ...
    'response.output_item.added': (event) => {
      ...
      console.log('[DEBUG] response.output_item.added - Response ID:', response_id, 'Item ID:', item.id, 'Type:', item.type);
      console.log('[DEBUG] Items currently in lookup:', Object.keys(this.itemLookup));
      ...
      console.log('[DEBUG] Adding item ID to response output:', item.id);
      response.output.push(item.id);
      
      // Create item for function_call and function_call_output since conversation.item.added doesn't fire for them
      if (!this.itemLookup[item.id]) {
        console.log('[DEBUG] Creating item from response.output_item.added');
        const newItem = JSON.parse(JSON.stringify(item));
        if (!newItem.content) {
          newItem.content = [];
        }
    ...
    'response.content_part.added': (event) => {
      ...
      console.log('[DEBUG] response.content_part.added - Looking for item:', item_id);
      console.log('[DEBUG] Items currently in lookup:', Object.keys(this.itemLookup));
      const item = this.itemLookup[item_id];
      if (!item) {
        console.error('[DEBUG] Item not found! Full event:', event);
        console.error('[DEBUG] All items:', this.items);
        throw new Error(
          `response.content_part.added: Item "${item_id}" not found`,
        );
      }
      console.log('[DEBUG] Found item:', item);
      console.log('[DEBUG] Item content before push:', item.content);
      item.content.push(part);
      console.log('[DEBUG] Item content after push:', item.content);
      return { item, delta: null };
    },
